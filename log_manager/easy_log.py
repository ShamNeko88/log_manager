...
class LogHandler():
    ...
    def __init__(
        ...
        # フォーマット（ファイル）
        self.log_file_format = logging.Formatter(
            "[%(levelname)s] %(asctime)s -%(message)s <場所: %(filename)s/%(funcName)s/%(lineno)d行目>"
        )
        # フォーマット（コンソール）
        self.log_console_format = logging.Formatter(
            "[%(levelname)s] %(asctime)s -%(message)s <場所: %(filename)s/%(funcName)s/%(lineno)d行目>"
        )
        # loggerの作成
